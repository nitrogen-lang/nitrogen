...
func TestHashIntAssignments(t *testing.T) {
	input := `m[0] = "value";`
...
func TestHashDotOperator(t *testing.T) {
	input := `m.value;`
	...
	exp, ok := stmt.Expression.(*ast.AttributeExpression)
	if !ok {
		t.Fatalf("exp is not ast.AttributeExpression. got=%T",
			stmt.Expression)
	}
